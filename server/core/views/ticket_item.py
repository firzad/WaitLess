...
class TicketItemByTicket(Resource):
    ...
    def get(self, ticket_id):
        ...
        ticket_item = TicketItemModel.query.filter(
            TicketItemModel.ticket_id == ticket_id).all()
        ...
        if 'item_status' in request.json:
            if 'order_item_ids' in request.json:
                for order_item_id in request.json['order_item_ids']:
                    ticket_item = TicketItemModel.query.get_or_404(
                        order_item_id)
    ...
    def get(self, session_id):
        ...
        for t in tickets:
            ...
            for ticket_item in ticket_items:
                menu_entry = MenuItemById().get_no_marshal(
                    ticket_item.menu_id)[0]
                menu_marsh = marshal(menu_entry, menu_resource_fields)
                ticket_item_marsh = marshal(
                    ticket_item, ticket_item_resource_fields)
                ticket_marsh = marshal(t, ticket_resource_fields)
                ticket_total.append(
                    {**ticket_marsh, **ticket_item_marsh, **menu_marsh})
...
class TicketSummary(Resource):
    def get(self):
        ...
        dishes_count = TicketItemModel.query.with_entities(TicketItemModel.menu_id, func.count(
            TicketItemModel.menu_id)).group_by(TicketItemModel.menu_id).all()
